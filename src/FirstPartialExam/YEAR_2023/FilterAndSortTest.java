        ...
        if (testCase == 1) { // students
            ...
            try {
                if (studentScenario == 1) {
                    ...
                    List<Student> filteredStudents = FilterAndSort.execute(students, (s) -> s.labAssistantPoints() >= 8.0 && s.getYear() >= 3);
                    ...
                    List<Student> filteredStudents = FilterAndSort.execute(students, (s) -> (s.grades.size() > s.totalCourses() * 0.9) && s.average() >= 9.0);
                    filteredStudents.forEach(System.out::println);
                }
            ...
            try {
                List<Integer> filteredIntegers = FilterAndSort.execute(integers, (i) -> (i % 2 == 0) && (i % 15 == 0));
