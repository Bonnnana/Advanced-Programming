...
class Scheduler<T> {
    ...
    public Timestamp<T> next() {
        ...
        return elements.stream()
                .filter(timestamp -> timestamp.getTime().isAfter(now))
                .min(Timestamp::compareTo)
                .orElseThrow(() -> new NoSuchElementException("No upcoming elements in the scheduler."));
    }

    public Timestamp<T> last() {
        LocalDateTime now = LocalDateTime.now();
        return elements.stream()
                .filter(timestamp -> timestamp.getTime().isBefore(now))
                .max(Timestamp::compareTo)
                .orElseThrow(() -> new NoSuchElementException("No past elements in the scheduler."));
    }

    List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        return elements.stream()
                .filter(el -> el.getTime().isBefore(end) && el.getTime().isAfter(begin))
