...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    void addGenericItem(String category, T element) {
        elements.computeIfAbsent(category, el -> new TreeSet<>());
    ...
    Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return elements.values().stream()
                ...
                .filter(value -> value.getTimestamp().isAfter(from) && value.getTimestamp().isBefore(to))
    ...
    public Collection<T> itemsFromCategories(List<String> categories) {
        return elements.entrySet()
                .stream()
                .filter(el -> categories.contains(el.getKey()))
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return elements.values().stream()
                ...
                .collect(Collectors.groupingBy(
                        el -> String.format("%02d-%02d", el.getTimestamp().getMonth().getValue(), el.getTimestamp().getDayOfMonth()),
                        TreeMap::new,
    ...
    public Map<Integer, Long> countByYear() {
        return elements.values().stream()
                .flatMap(el-> el.stream())
                .collect(Collectors.groupingBy(
                        value -> value.getTimestamp().getYear(),
                        TreeMap::new,
                        Collectors.counting()
                ));
